...
import {
  coreConfig,
  isPlatformEmulationReady,
  runnerConfig,
} from "@/utils/retroarch";
...
function ContinuePlayingRom({
  ...
  const romDownload = useMemo(() => getRomDownload(rom.id), [
    rom.id,
    getRomDownload,
  ]);
  const isDownloadFinished = romDownload?.event === "cancelled" ||
    romDownload?.event === "finished";
  ...
  const getCtaButton = useCallback(() => {
    const isReadyToPlay = isDownloaded &&
  ...
  return (
    <div
      className={clsx([
        "grid grid-cols-[14.8rem_16.812rem] max-w-[31.688rem] w-full",
        className,
      ])}
    >
      <GameCover
      ...
      <div className="flex flex-col pt-[1.375rem] px-[1.375rem] justify-between">
        {hideTitle ? <div>&nbsp;</div> : (
          <a href={romURL}>
            <Heading
              variant={rom.name.length > 21 ? "h3" : "h2"}
                ...
                <div className="flex gap-2 w-full flex-col">
                  ...
                  >
                    <DownloadStatus
                      ...
                      romSizeBytes={rom.fsSizeBytes}
                    />
                  </div>
                  <Progress
                    className={clsx(["flex w-full max-h-[0.25rem]"])}
                    value={getProgress()}
                  />
                </div>
