...
export default function Score({igdbScore}: {igdbScore?: number}) {
	console.log(igdbScore)
	return (
		<ContentCard title='Rating' contentClassName='flex flex-col gap-4 justify-center'>
			{igdbScore != null && (
				<ScoreProgressBar
					value={igdbScore}
...
const ScoreProgressBar = ({
	...
	const rounded = useTransform(() => Math.trunc(animatedValue.get()))
	...
	return (
		<div className='flex gap-6 items-center'>
			...
			<div className='relative w-full'>
				<Progress value={value} className={progressColor} indicatorClassName={indicatorColor} />
				<motion.span
					initial={{x: -20}}
					animate={{x: 0}}
					transition={{type: 'tween', duration: 0.5}}
					style={{right: `${100 - (value || 0) - 5}%`}}
					className={clsx([
						`
							absolute
							flex 
