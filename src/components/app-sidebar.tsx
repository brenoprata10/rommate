...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const platformMenuItem: NavItem = React.useMemo(
    () => ({
      ...
      icon: Gamepad2,
      url: "#",
      items: platforms && !platformsError
        ? platforms
          ...
          .reduce(
            (result, platform) => {
              const isPlatformDuplicated = result.some(
                ({ title }) =>
                  title.toUpperCase() === platform.title.toUpperCase(),
              );
  ...
  const collectionMenuItem: NavItem = React.useMemo(
    () => ({
      ...
      icon: SquaresUnite,
      url: "#",
      items: collections && !collectionsError
  ...
  return (
    <Sidebar collapsible="icon" {...props}>
      <SidebarHeader>
        <NavLink
          className="p-2 flex items-center gap-2 font-medium text-lg overflow-hidden"
          to={"/"}
        >
