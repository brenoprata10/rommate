...
use crate::{
    dtos::save_sync::{FileConflictInfo, SaveSync, SaveSyncKind},
    enums::error::Error,
    models::rom::{Rom, RomUserSave},
    romm::romm_http::RommHttp,
    services::{downloader::DownloaderService, file::FileService, rom::RomService},
};
...
impl RomSaveService {
    pub async fn get_rom_saves(
        ...
        let response = RommHttp::get(app_handle, &format!("/api/saves?rom_id={}", rom_id))?
            .send()
            .await?;
    ...
    pub async fn get_most_recent_rom_save(
        app_handle: &AppHandle,
        rom_id: i32,
    ) -> Result<RomUserSave, Error> {
        ...
        let latest_save = rom_saves
            .into_iter()
            .last()
            .ok_or(Error::NotFound("No saves were returned".to_string()))?;

        Ok(latest_save)
    }

    pub async fn download_most_recent_save_file(
        app_handle: &AppHandle,
        rom_id: i32,
    ) -> Result<(), Error> {
    ...
    pub async fn upload_save_file(
        ...
        if let Some((screenshot_file, screenshot_path)) = screenshot {
            ...
            form = form.part(
                "screenshotFile",
                multipart::Part::bytes(screenshot_content)
                    .file_name(FileService::get_file_name(screenshot_path)?)
                    .mime_str("image/png")?,
            );
        };

        let response = RommHttp::post_multipart(app_handle, &url, form)?
            .send()
            .await?;
    ...
    pub async fn check_save_sync(app_handle: &AppHandle, rom_id: i32) -> Result<SaveSync, Error> {
        ...
        let file_url = format!("/api/saves/{}/content", latest_save.id);
        let mut uploaded_save =
            DownloaderService::temporary_file(RommHttp::get(app_handle, &file_url)?).await?;

        let rom_path = format!(
            "{}/{}.{}",
            platform_fs_slug, fs_name_no_ext, latest_save.file_extension
        );
        let mut local_save = match File::open(format!(
            "{}/{}",
        ...
        if FileService::is_equal(&mut uploaded_save, &mut local_save).await? {
            return Ok(SaveSync {
